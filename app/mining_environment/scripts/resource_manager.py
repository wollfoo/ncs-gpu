...
class SharedResourceManager:
    ...
    **Handles** (xử lý – đảm nhiệm):
    ...
    def __init__(self, config: ConfigModel, logger: logging.Logger, resource_managers: Dict[str, Any]):
        ...
        # Lazy GPUResourceManager instance (SSOT for metrics)
        self._grm = None
        
        # NVML lifecycle has been decoupled from SharedResourceManager.
        # Derive initialization state from GPUResourceManager (SSOT) if available.
        try:
            grm = self._get_grm()
            if grm is not None:
                self._nvml_init = bool(getattr(grm, 'gpu_initialized', False))
                self._nvml_available = self._nvml_init
                self.logger.info("SharedResourceManager initialized (GPUResourceManager manages GPU/NVML state)")
            else:
                self.logger.info("SharedResourceManager initialized (GPUResourceManager unavailable; NVML state unknown)")
        except Exception as e:
            self.logger.debug(f"Error deriving GPU state from GPUResourceManager: {e}")
            self.logger.info("SharedResourceManager initialized (defensive defaults; NVML lifecycle removed)")

    def is_nvml_initialized(self) -> bool:
        """Backward-compatible check mapped to GPUResourceManager.gpu_initialized."""
        try:
            grm = self._get_grm()
            return bool(grm and getattr(grm, 'gpu_initialized', False))
        except Exception:
            return False

    def initialize_nvml(self) -> None:
        """Deprecated: NVML lifecycle is managed by GPUResourceManager. No-op."""
        try:
            grm = self._get_grm()
            self._nvml_init = bool(grm and getattr(grm, 'gpu_initialized', False))
            self._nvml_available = self._nvml_init
            self.logger.info("[DEPRECATED] initialize_nvml(): managed by GPUResourceManager → no-op")
        except Exception as e:
            self.logger.debug(f"[DEPRECATED] initialize_nvml() error: {e}")
    ...
    def shutdown_nvml(self):
        ...
        try:
            self._nvml_init = False
            self._nvml_available = False
            self.logger.info("[DEPRECATED] shutdown_nvml(): managed by GPUResourceManager → no-op")
        except Exception as e:
            self.logger.debug(f"[DEPRECATED] shutdown_nvml() error: {e}")

    def _get_grm(self):
        """
        Lazy import và khởi tạo GPUResourceManager (nguồn chân lý duy nhất cho GPU metrics).
        Trả về instance nếu thành công; None nếu lỗi.
        """
        if getattr(self, "_grm", None) is not None:
            return self._grm
        try:
            try:
            ...
            self.logger.debug(f"Không thể khởi tạo GPUResourceManager: {e}")
            self._grm = None
            return None
    ...
    def _sync_get_gpu_usage_percent(self, pid: int) -> float:
        ...
        try:
            ...
            if grm is None:
                return 0.0
            # Ánh xạ PID → GPU index bằng GPUResourceManager
            gpu_index = None
            try:
                gpu_index = grm.infer_gpu_index_for_pid(pid)
            except Exception as _e:
                # Không chặn, chỉ log nhẹ
                self.logger.debug(f"infer_gpu_index_for_pid lỗi cho PID {pid}: {_e}")
                gpu_index = None
            if gpu_index is None:
                return 0.0
            # Lấy snapshot (SSOT) và trích xuất utilization ratio (0..1) → phần trăm
            snap = grm.get_metrics_snapshot()
            util_ratio = None
            try:
                util_ratio = snap.utilization.get(int(gpu_index))
            except Exception:
                util_ratio = None
            if isinstance(util_ratio, (int, float)) and util_ratio is not None:
                return float(util_ratio) * 100.0
            return 0.0
        except Exception as e:
