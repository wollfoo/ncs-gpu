...
class HandoffCoordinator:
    """
    **Handoff Coordination Manager** (trình quản lý phối hợp chuyển giao)
    
    Prevent rapid successive handoffs that cause race conditions.
    Implements timing protection and thread-safe handoff operations.
    
    Features:
    - **Minimum Handoff Interval**: 1.0 second between handoffs cho cùng PID
    - **Handoff Timing Protection**: Track last handoff time per PID
    - **Coordination Lock**: Thread-safe handoff operations
    - **Enhanced Logging**: Detailed handoff operation tracking
    """
    
    def __init__(self):
        """Initialize HandoffCoordinator với timing protection"""
        # **Thread-safe handoff coordination** (phối hợp chuyển giao an toàn luồng)
        self._handoff_lock = threading.RLock()
        self._last_handoff_time: Dict[int, float] = {}  # PID -> timestamp
        self._minimum_handoff_interval = 1.0  # seconds
        
        # **Handoff statistics** (thống kê chuyển giao)
        self._handoff_stats = {
            'total_handoffs': 0,
            'delayed_handoffs': 0,
        ...
        # **Active handoffs tracking** (theo dõi chuyển giao đang hoạt động)
        self._active_handoffs: Dict[int, float] = {}  # PID -> start_time
        
        logger.info("🤝 HandoffCoordinator initialized with 1.0s minimum interval")
    
    def coordinate_handoff(self, pid: int, handoff_func: Callable, *args, **kwargs) -> bool:
        """
        **Coordinate Handoff Operation** (phối hợp hoạt động chuyển giao)
        
        Core method để coordinate handoff operations với timing protection.
        Ensures minimum interval between handoffs for same PID.
        
        Args:
            pid: Process ID being handed off
            handoff_func: Function thực thi handoff operation
            *args, **kwargs: Arguments cho handoff function
            
        Returns:
            bool: True nếu handoff successful, False otherwise
        """
        with self._handoff_lock:
            try:
                # **Check if handoff is too soon** (kiểm tra chuyển giao quá sớm)
                now = time.time()
                last_time = self._last_handoff_time.get(pid, 0)
                time_since_last = now - last_time
                
                if time_since_last < self._minimum_handoff_interval:
                    wait_time = self._minimum_handoff_interval - time_since_last
                    logger.warning(
                        f"⏰ [HANDOFF-COORD] Handoff too soon for PID {pid}, "
                        f"waiting {wait_time:.3f}s (last: {last_time:.3f}, now: {now:.3f})"
                    )
                    time.sleep(wait_time)
                    self._handoff_stats['delayed_handoffs'] += 1
                
                # **Mark handoff as active** (đánh dấu chuyển giao đang hoạt động)
                self._active_handoffs[pid] = time.time()
                
                # **Execute handoff with timing tracking** (thực thi chuyển giao với theo dõi thời gian)
                start_time = time.time()
                logger.info(f"🤝 [HANDOFF-COORD] Starting coordinated handoff for PID {pid}")
                
                # Execute the actual handoff function
                result = handoff_func(*args, **kwargs)
                
                # **Update handoff timing** (cập nhật thời gian chuyển giao)
                end_time = time.time()
                self._last_handoff_time[pid] = end_time
                handoff_duration = end_time - start_time
                
                # **Update statistics** (cập nhật thống kê)
                self._handoff_stats['total_handoffs'] += 1
                if result:
                    self._handoff_stats['successful_handoffs'] += 1
                    logger.info(
                        f"✅ [HANDOFF-COORD] Coordinated handoff successful for PID {pid} "
                        f"(duration: {handoff_duration:.3f}s)"
                    )
                else:
                    self._handoff_stats['failed_handoffs'] += 1
                    logger.error(
                        f"❌ [HANDOFF-COORD] Coordinated handoff failed for PID {pid} "
                        f"(duration: {handoff_duration:.3f}s)"
                    )
                ...
                logger.error(f"❌ [HANDOFF-COORD] Handoff coordination exception for PID {pid}: {e}")
                self._handoff_stats['failed_handoffs'] += 1
                return False
            
            finally:
                # **Clean up active handoff tracking** (dọn dẹp theo dõi chuyển giao đang hoạt động)
                self._active_handoffs.pop(pid, None)
    
    def is_handoff_allowed(self, pid: int) -> tuple[bool, float]:
        """
        **Check Handoff Timing** (kiểm tra thời gian chuyển giao)
        
        Check if handoff is allowed based on timing constraints.
        
        Args:
            pid: Process ID to check
            
        Returns:
            tuple[bool, float]: (is_allowed, time_to_wait)
        """
        with self._handoff_lock:
            now = time.time()
            last_time = self._last_handoff_time.get(pid, 0)
    ...
    def get_handoff_statistics(self) -> Dict[str, Any]:
        """
        **Get Handoff Statistics** (lấy thống kê chuyển giao)
        
        Returns:
            Dict containing handoff coordination statistics
        """
        with self._handoff_lock:
            return {
                **self._handoff_stats,
                'active_handoffs': len(self._active_handoffs),
                'minimum_interval': self._minimum_handoff_interval,
                'tracked_pids': len(self._last_handoff_time)
            }
    
    def reset_handoff_timing(self, pid: int) -> bool:
        """
        **Reset Handoff Timing** (reset thời gian chuyển giao)
        
        Reset timing tracking for specific PID. Used for testing or error recovery.
        
        Args:
            pid: Process ID to reset
        ...
        try:
            with self._handoff_lock:
                if pid in self._last_handoff_time:
                    del self._last_handoff_time[pid]
                    logger.info(f"🔄 [HANDOFF-COORD] Reset handoff timing for PID {pid}")
                    return True
                else:
                    logger.warning(f"⚠️ [HANDOFF-COORD] No timing data found for PID {pid}")
                    return False
        except Exception as e:
            logger.error(f"❌ [HANDOFF-COORD] Failed to reset timing for PID {pid}: {e}")
            return False
